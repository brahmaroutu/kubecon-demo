...
type Controller struct {
	...
	serviceLister svclisters.ServiceLister
	deploymentsSynced cache.InformerSynced
	barsLister        barlisters.BarLister
...
func NewController(
	...
	// types.
	deploymentInformer := kubeInformerFactory.Core().V1().Services()
	...
	controller := &Controller{
		...
		serviceLister:     deploymentInformer.Lister(),
	...
	barInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		DeleteFunc: controller.enqueueBar,
	})
...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		// The Bar resource may no longer exist, in which case we delete the service
        ...
        if deploymentName == "" {
                ...
                runtime.HandleError(fmt.Errorf("%s: service name must be specified", key))
                return nil
        }

        // Get the deployment with the name specified in Foo.spec
        service, err := c.serviceLister.Services(bar.Namespace).Get(deploymentName)
        // If the resource doesn't exist, we'll create it
        if errors.IsNotFound(err) {
                service, err = c.kubeclientset.CoreV1().Services(bar.Namespace).Create(newService(bar))
        }

	//Srinib Upon Bar creation we create service
...
func (c *Controller) updateBarStatus(bar *controlleev1alpha1.Bar, deployment *appsv1.Deployment) error {
	...
	//TODO srinib barCopy.Status.AvailableReplicas = deployment.Status.AvailableReplicas
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		if err != nil {
			glog.V(4).Infof("ignoring orphaned object '%s' of bar '%s'", object.GetSelfLink(), ownerRef.Name)
			return
		}
...
// newService creates a new Service for a Bar resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the Bar resource that 'owns' it.
func newService(bar *controlleev1alpha1.Bar) *api.Service {
	port, err := strconv.ParseInt(bar.Spec.Selector.MatchLabels["svcport"],10,32)
	if err != nil {
	  port = 8080
	}
	fmt.Println(port)
        return &api.Service {
			...
			//TODO srinib add a selector app = bar.Spec.DeploymentName?
                },
        }
}
